...
# not quite right, contact calculation is too coarse, cannot just be based on distance
...
def evaluate_episode(model: torch.nn.Module, dataset, epi_idx: int, print_every: int = 20):
    ...
    if dataset.use_relative_action:
        print(gt_plot.shape, pred_plot.shape, o[1:-1].shape)
        pred_plot = dataset.relative_action_to_global(o[1:-1].detach().cpu(), torch.tensor(pred_plot)).numpy()
        gt_plot = dataset.relative_action_to_global(o[1:-1].detach().cpu(), torch.tensor(gt_plot)).numpy()
    ...
    for d in range(gt_plot.shape[1]):
        axes[1].plot(steps, gt_plot[:, d],  colors[d], label=f'GT d{d}', linewidth=2)
        axes[1].plot(steps, pred_plot[:, d], colors[d]+'--', label=f'Pred d{d}', linewidth=2)
