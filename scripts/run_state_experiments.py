#!/usr/bin/env python3
"""Batch runner for state-based GPI experiments with concise reporting."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run randomised state-based GPI experiments.")
...
def generate_configs(count: int, seed: Optional[int]) -> list[ExperimentConfig]:
...
def append_log(summaries: Iterable[tuple[ExperimentConfig, ExperimentResult]]) -> None:
...
def main() -> None:
    ...
    summaries: list[tuple[ExperimentConfig, ExperimentResult]] = []
    for config in configs:
        set_global_seed(config.random_seed)
        result = run_experiment(config, dataset, args.max_steps)
